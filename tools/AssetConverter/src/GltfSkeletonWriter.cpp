...
GltfSkeletonStatus WriteSkeletonGltf(const LabAnimation& anim,
    ...
    //
    // Обратные bind-матрицы НЕ избыточны: проверено численно на 0301.lab, что
    // поза первого кадра отличается от bind-позы (произведение world(кость) на
    // IBM отклоняется от единичной матрицы на 2.0 — это не погрешность).
    // Выбросить их — значит потерять данные, восстановить которые из дорожки
    // невозможно.
    //
    // Побочный эффект: на skin не ссылается ни один меш, и импортёр Blender
    // подставляет для него объект-заглушку (икосферу на 42 вершины). Она
    // безвредна и удаляется вручную; целостность данных важнее.
    json.Key("skins");
    json.BeginArray();
    json.BeginObject();
