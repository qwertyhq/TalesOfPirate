...
def texture_name_for(material_name):
    ...
    имени ассета точка недопустима. К суффиксу он добавляет порядковый номер,
    когда материалов с одним именем несколько: `010038_bmp1`. Без учёта этого
    номера у таких материалов текстура не находилась вовсе, и постройки
    оставались серыми.
    """
    return re.sub(r"_(bmp|dds|png|tga|jpg|jpeg)\d*$", "", material_name,
                  flags=re.IGNORECASE)
